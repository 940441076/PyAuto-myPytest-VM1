...
class Test_SystemSettingPage:
    ...
    def test_selfTest(self):
        ...
        try:
            app = common_util.connect_application()
            common_util.back_systemSettingPage()
            with allure.step('非工程师模式不显示硬件自检按钮'):
                time.sleep(1)
                common_util.screen_shot('不显示硬件自检按钮')
                app['血管内断层成像系统'].child_window(title="硬件自检", auto_id="btnSelfTest",control_type="Button").wait_not('visible',timeout=3)
                assert True
            with allure.step('打开工程师模式'):
                show_test = app['血管内断层成像系统'].child_window(title="显示工程师模式", auto_id="btnEngineerMode",
                                                                   control_type="Button")
                rect = show_test.rectangle().mid_point()
                mouse.click(coords=(rect.x, rect.y))
